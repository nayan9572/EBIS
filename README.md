...
> 📄 **Current Research Status:** See `PROJECT_STATUS_AUGUST_2026.md` for the latest forensic audit findings, and `EBIS_Runtime_Stabilization_v25_Report.md` for the autonomous closed-loop stabilization case study — both summarized below.
...
Full detail for both sessions — including the complete v25 engineering narrative — is in `PROJECT_STATUS_AUGUST_2026.md`. The unedited runtime transcript is preserved verbatim in `EBIS_Runtime_Stabilization_v25_Report.md`.
...
- **`PROJECT_STATUS_AUGUST_2026.md`** — Current forensic causal-discovery audit (v227): reproducibility, epistemic decomposition, and prediction verification.
- **`EBIS_Runtime_Stabilization_v25_Report.md`** — Complete verbatim runtime transcript for the v25 autonomous stabilization session (causal discovery, autonomous experimentation, and closed-loop actuator control on `ca50`) — the project's current closed-loop control milestone.
