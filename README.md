...
- [`PROJECT_STATUS_AUGUST_2026.md`](./PROJECT_STATUS_AUGUST_2026.md) — Current status report: this month's closed-loop stabilization session (v25) as the main body — autonomous actuator selection, a failed attempt, self-healing graph refresh, and a verified pass — with the earlier discovery-only session (v227) summarized in an appendix for context.
- [`EBIS_Runtime_Stabilization_v25_Report.md`](./EBIS_Runtime_Stabilization_v25_Report.md) — Complete verbatim runtime transcript for the v25 autonomous stabilization session (causal discovery, autonomous experimentation, and closed-loop actuator control on `ca50`) — the project's current closed-loop control milestone, and the primary evidence source for the status report above.
- [`EBIS_v227_Annotated_Audit_Trail.pdf`](./EBIS_v227_Annotated_Audit_Trail.pdf) — Full annotated log for the v227 forensic session, recorded July 2026 (41 raw log blocks, independently annotated). Discovery-and-prediction only — no actuator experiments; summarized in the status report's appendix, with the full annotated version here.
